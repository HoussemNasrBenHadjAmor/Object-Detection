...
def save_recall_confidence_curve(coco_evaluator, category_ids, category_names, out_dir, title):
    ...
    scores = coco_evaluator.coco_eval['bbox'].eval['scores']
    ...
    for idx, cat_id in enumerate(category_ids):
        ...
        adjusted_idx = cat_id - 1    
        
        # Fetch recall and scores for the category
        recall_at_cat = recall[:, adjusted_idx, 0, -1]
        scores_at_cat = scores[:, :, adjusted_idx, 0, -1].mean(axis=1)

        # Filter valid scores and recalls
        valid_indices = scores_at_cat > -1
        valid_scores = scores_at_cat[valid_indices]
        valid_recalls = recall_at_cat[valid_indices]


        if len(valid_recalls) > 0 and len(valid_scores) > 0:
            plt.plot(valid_scores, valid_recalls, label=f'{category_names[cat_id]["name"]}')
...
def save_f1_confidence_curve(coco_evaluator, category_ids, category_names, out_dir, title):
    recall = coco_evaluator.coco_eval['bbox'].eval['recall']
    scores = coco_evaluator.coco_eval['bbox'].eval['scores']
    ...
    for idx, cat_id in enumerate(category_ids):
        ...
        recall_at_cat = recall[:, adjusted_idx, 0, -1]
        scores_at_cat = scores[:, :, adjusted_idx, 0, -1].mean(axis=1)
        precision_at_cat = precision[:, :, adjusted_idx, 0, -1].mean(axis=1)

        # Filter valid scores, precisions, and recalls
        valid_indices = scores_at_cat > -1
        valid_scores = scores_at_cat[valid_indices]
        valid_precisions = precision_at_cat[valid_indices]
        valid_recalls = recall_at_cat[valid_indices]

        f1_scores = 2 * (valid_precisions * recall_at_cat) / (valid_precisions + recall_at_cat + 1e-6)

        if len(valid_recalls) > 0 and len(valid_scores) > 0 and len(valid_precisions) > 0 :
            plt.plot(valid_scores, f1_scores, label=f'{category_names[cat_id]["name"]}')
...
def save_precision_confidence_curve(coco_evaluator, category_ids, category_names, out_dir, title):
    ...
    for idx, cat_id in enumerate(category_ids):
        ...
        # Fetch precision and scores for the category
        precision_at_cat = precision[:, :, adjusted_idx, 0, -1].mean(axis=1)
        scores_at_cat = scores[:, :, adjusted_idx, 0, -1].mean(axis=1)
        ...
        if len(valid_precisions) > 0 and len(valid_scores) > 0:
            plt.plot(valid_scores, valid_precisions, label=f'{category_names[cat_id]["name"]}')
...
def save_precision_recall_curve(coco_evaluator, category_ids, category_names, out_dir, title):
    precision = coco_evaluator.coco_eval['bbox'].eval['precision']
    ...
    for idx, cat_id in enumerate(category_ids):
        ...
        precision_at_cat = precision[:, :, idx, 0, -1].mean(axis=1)
        recall_at_cat = recalls[:, idx, 0, -1]

        # Filter valid scores, precisions, and recalls
        valid_indices = precision_at_cat > -1
        valid_precisions = precision_at_cat[valid_indices]
        valid_recalls = recall_at_cat[valid_indices]

        #print(f"recall_at_cat for idx : {recall_at_cat} --> {idx} ")
        #ap = precision_at_cat.mean(axis=1)  # average precision over IoU thresholds
        #print(f"ap : {ap}")
        #precision_at_cat = precision_at_cat[precision_at_cat > -1]  # filter out invalid values

        if len(valid_precisions) > 0 and len(valid_recalls) > 0 :
            plt.plot(valid_recalls, valid_precisions, label=f'{category_names[cat_id]["name"]} (AP={valid_precisions.mean():.3f})')
